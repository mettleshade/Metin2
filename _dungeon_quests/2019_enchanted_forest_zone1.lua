quest enchanted_forest_zone begin
    state start begin
		...
		when 20412.chat."Bagjanamu Odasý " begin
			...
			say("")
			say("")
			say_center("Bagjanamu'ya karþý savaþmak ister misin? ")
			...
			if agree == 1 then
				...
				if not party.is_party() then
					...
					say("")
					say("")
					say_center("Bu odaya eriþebilmek için ")
					say_center("grup üyesi olmanýz gerekmektedir. ")
					return
				end
				...
				if d.find(party.getf("frt_dungeon_index")) and party_check then
					if get_global_time() > pc.getqf("exit_time_logout") then
						...
						say("")
						say("")
						say_center("Maalesef çok geç geliyorsun, giriþi kýsýtlýyorsun ")
						say_center("Zindana dönüþ 5 dakýkalýk. ")
						return
					end
					...
					say("")
					say("")
					say_center("Grubunuz Iþýnlanýyor. ")
					say("")
					wait()
				...
				if game.get_event_flag(event_flag) > get_time() then
					...
					say("")
					say("")
					say_center("Giriþ Þuanda Kapalý.")
				...
				if not party.is_leader() then
					...
					say("")
					say_center("Bu odaya eriþebilmek için ")
					say_center("grup lideri olmanýz gerekmektedir. ")
					return
				end
				...
				if party.get_near_count() < 2 then
					...
					say("")
					say_center("Bu odaya eriþebilmek için ")
					say_center("2 adet grup üyesi olmasý gerekmektedir. ")
					return
				end
				...
				if not ticketCheck then
					...
					say("")
					say_center("Bu odaya eriþebilmek için ")
					say_center(string.format("Tüm Grupta Olmasý Gereken Eþya - %s. ", item_name(setting.ticket_vnum)))
					say_center("")
					say_center("Gruptaki bazý üyelerin bileti yok ! ")
				...
				say("")
				say("")
				say_center("Devam ettikten sonra tüm ")
				say_center("grup üyeleri odaya ýþýnlanmýþ olacak. ")
				wait()
