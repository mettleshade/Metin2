quest spider_dungeon_3floor_boss begin
	...
	state boss_ok begin
		...
		when 30130.chat." Yuvaya giriþ " begin
			...
			if game.get_event_flag("spider_lair_ongoing_"..get_channel_id()) == 1 then
				...
				if party.get_leader_pid() == game.get_event_flag("spider_lair_leader_"..get_channel_id()) then
					...
					if  starttime +  300 >= current_time  and party.get_leader_pid() == game.get_event_flag("spider_lair_leader_"..get_channel_id()) and game.get_event_flag("spider_lair_ongoing_"..get_channel_id()) == 1 then
						...
						say("")
						say("")
						say_center("Grup liderin yuvaya girdi. Sen de içeriye[ENTER]girip Barones ile savaþmak istiyor musun? ") -- Å×½ºÆ®¿ë
						...
						say("")
						say("")
						say_center("Süre bitti. Anahtar parçalandý.[ENTER]Yeniden Anahtar üretmelisin. ") -- Å×½ºÆ®¿ë
						return
					end
				else
					...
					if remain_time < 0 then
						...
						say("")
						say("")
						say_center("Bu iþe yaramadý. Lütfen tekrar deneyin!")
						game.set_event_flag("spider_lair_ongoing_"..get_channel_id(),0)
						game.set_event_flag("spider_lair_leader_"..get_channel_id(), 0)
						game.set_event_flag("spider_dead"..get_channel_id(),0)
						return
					...
					say("")
					say("")
					say_center("Þu an baþka bir grup yuvada Barones ile[ENTER]savaþýyor. Onlarýn savaþý bitirmesini bekle. ")
					say_center(string.format("Önceki grup içeriden çýktýktan sonra girebilirsin: Kalan zaman %s[ENTER]dakika. ", math.ceil(remain_time)))
					return
				end	
			elseif  pc.getqf("retry_limit_time") > get_global_time() then
				...
				say("")
				say("")
				say_center("Son savaþýn üzerinden en az bir saat geçmeesi[ENTER]gerekiyor. Bu süree içesinde tekrar Barones'in[ENTER]yuvasýna giremezsin.Kalan zaman:")
			...
			elseif party.is_leader()   then
				 if false == pc.can_warp() then
					...
					say("")
					say("")
					say_center("Þu anda bu alana giremezsin.[ENTER]Lütfen biraz bekleyin.")
					return
				 end
	...
	state inLair begin
		...
		when 2092.kill begin
			...
			notice_all(string.format("%s Grubu Örümcek Baronesi Öldürdü!", pc.get_name()))
		end
