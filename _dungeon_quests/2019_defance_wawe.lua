	...
	state start begin
		...
		when 20436.chat."Dalgayý Baþlat" begin
			if party.is_leader() then
				say_npc()
				say("")
				say("Bir grupta iken giriþ yapamazsýn!")
			else
				say_npc()
				say("")
		...
		when 20013.chat."Herkes Gemiye!" with (game.get_event_flag("enable_defance_wave_system") == 0) begin
			...
			-- if party.get_near_count() < 2 then
				...
				-- say("")
				-- say("")
				-- say_reward_center("Bu gemiye en az 2 kiþi gelmelisin.")
				-- say("")
				-- say_reward_center("Yanýnda bulunan üye sayýsý : "..party.get_near_count())
				-- say_reward_center("Yanýnda bulunan üye sayýsý : "..party.get_near_count())
				-- return
			-- end
			...
			-- if not repeatDungeontime then
				...
				-- say("")
				-- say("")
				-- say_center("Uyarý! Gemi savunmasýndan yeni çýkmýþ grup[ENTER]üyeleri mevcut aþaðýdaki üyeler 6 saat[ENTER]sonra giriþ yapabilir")
			...
			-- if not ticketCheck then
				...
				-- say("")
				-- say("")
				-- say_center("Uyarý! Gemi savunmasýna girebilmek için,[ENTER]bütün grup üyelerinin[ENTER]geçit biletine sahip olmasý gereklidir.")
			...
			-- if not levelCheck then
				...
				-- say_center("Uyarý! Gemi savunmasýna girebilmek için,[ENTER]bütün grup üyelerinin[ENTER]100.Seviye'ye ulaþmýþ olmasý gereklidir.")
			...
			if (get_time() < pc.getf("defensewave", "dungeon_last_login_time")) then
				...
				say_center("Kalan süre: "..dungeon_get_duration(pc.getf("defensewave", "dungeon_last_login_time") - get_global_time()).."")
				say_center("")
				return
			end
			...
			if game.get_event_flag("mashroom_dung_close") == 1 then
				...
				say_center("")
				say_center("")
				say_center("Lanetli Orman henüz aktif durumda deðildir.")
				say_center("")
				return
			else
				...
				if game.get_event_flag("enable_defance_wave_system") == 1 then
					...
					say("")
					say("")
					say_reward_center("Harita þuanda bakým modunda.")
					return
				end
		...
		when 20013.chat."Süremi sýfýrlamak istiyorum.." with (game.get_event_flag("enable_defance_wave_system_time") == 1) begin
			...
			say("")
			say("")
			say_center("Gemi savunmasýna tekrar girebilmek için")
			say_center("Süreni sýfýrlaman gerekiyor")
			say("")
			say_reward_center("Süreni sýfýrlamak ister misin ?")
			...
			if select_time == 1 then
				...
				say("")
				say("")
				say_center("Süreni sýfýrlamam için bana")
				...
				if select_time_new == 1 then
					if (get_time() > pc.getf("defensewave", "dungeon_last_login_time")) then
						...
						say("")
						say("")
						say_reward_center("Zaten süren sýfýrlanmýþ durumda [ENTER]tekrar giriþ yapabilirsin.")
					elseif pc.get_gold() < 50000000 then
						...
						say("")
						say("")
						say_reward_center("Yeterli miktarda Yang'a sahip deðilsin.")
					elseif pc.is_busy() == true then
						...
						say("")
						say("")
						say_reward_center("Meþgul durumda olduðun için bunu gerçekleþtiremezsin.")
					elseif pc.is_dead() == true then
						...
						say("")
						say("")
						say_reward_center("Ölü durumda olduðun için bunu gerçekleþtiremezsin.")
					else
						say_title_center("Balýkcý: ")
						addimage(35,35,'hidra.tga')
						say("")
						...
						say_reward_center("Süren sýfýrlandý tekrar giriþ yapabilirsin.")
