quest pvp_manager begin
	state start begin
		when 20087.chat."GM:PvP Turnuvasýna Iþýnlan " with pc.is_gm() begin
		...
		when 20087.chat."PvP Turnuvasýna Katýl " with game.get_event_flag("pvp") == 1 begin
			say_title("PvP Turnuvasýna Katýl:")
				if pc.getqf("turnuva_giris_engeli") == 1 then
					say_reward("Yasak nesne kullanýmý olduðu için giriþiniz engellendi.")
					return
				elseif pc.count_item(39011) > 0 or pc.count_item(39013) > 0 then
					say_reward("Envanterinde "..item_name(39011).." veya "..item_name(39013).." bulundurmazsýn.")
				else
		...
		when 20078.chat."GM: Sýnýf Seçimi " with pc.get_map_index() == 500 and pc.is_gm() and game.get_event_flag("pvp") == 0 and ae_pvp_tournament == nil begin
			say("")
			say("Turnuva sýnýfýný seç.")
			say("")
			local sinif = select("Savaþçý ", "Ninja ", "Sura ", "Þaman ", "Karýþýk ", "Vazgeç ")
			if sinif == 1 then
				game.set_event_flag("pvp_sinif",1)
				say("Savaþçý sýnýfý seçildi.")
			elseif sinif == 2 then
				game.set_event_flag("pvp_sinif",2)
				say("Ninja sýnýfý seçildi.")
			elseif sinif == 3 then
				game.set_event_flag("pvp_sinif",3)
				say("Sura sýnýfý seçildi.")
			elseif sinif == 4 then
				game.set_event_flag("pvp_sinif",4)
				say("Þaman sýnýfý seçildi.")
			elseif sinif == 5 then
		...
		function open_registrations()
			...
			notice_all("Giriþler için "..mob_name(20087).."'e gidin.")
			local sinif = game.get_event_flag("pvp_sinif")
			local siniff = ""
			if sinif == 1 then
				siniff = "Savaþçý "
				notice_all(siniff.."Arasý Turnuva Baþladý. Level limiti: "..game.get_event_flag("pvp_minlevel").." - "..game.get_event_flag("pvp_maxlevel").."")
			elseif sinif == 2 then
				siniff = "Ninja "
				notice_all(siniff.."Arasý Turnuva Baþladý. Level limiti: "..game.get_event_flag("pvp_minlevel").." - "..game.get_event_flag("pvp_maxlevel").."")
			elseif sinif == 3 then
			...
			if sinif == 1 then
				if pc.get_job() != 0 then
					if pc.get_job() != 4 then
						say("Sadece savaþçýlar girebilir.")
						return
					end
				end
			elseif sinif == 2 then
			...
			if arena[pvp_manager.get_arena()][5] == 0 then
				...
				if math.mod(arena[pvp_manager.get_arena()][5], 5) == 0 then
					if tostring(arena[pvp_manager.get_arena()][5]) == "30" then
						...
						syschat("Ýtemlerini deðiþtirmek için 15 saniyen var.")
						syschat("15 saniye sonunda item deðiþtiremeyeceksin.")
